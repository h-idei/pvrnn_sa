...
class plot_rnn(object):
    ...
    def state(self, slice_num, tmin, tmax, width, height):
        ...
        #free energy
        meta_prior = 0.005
        fe = np.zeros((LENGTH, 1))
        fe_moving_average = np.zeros((LENGTH, 1))
        ...
        for i, filename in enumerate(self.state_filename_list):
            ...
            if i == 0:
                 fe[i] = er_proprio[i] + er_extero[i] + meta_prior * (data["top_kl"][0][0].detach().numpy()  + data["top_kl"][0][1].detach().numpy() + data["prop_kl"][0][0].detach().numpy() + data["vision_kl"][0][0].detach().numpy())
            else:
            	fe[i] = er_proprio[i] + er_extero[i] + meta_prior * (data["top_kl"][0][1].detach().numpy() + data["prop_kl"][0][0].detach().numpy() + data["vision_kl"][0][0].detach().numpy())
        ...
        for i in range(np.array(self.state_filename_list).shape[0], LENGTH):
            ...
            fe[i] = er_proprio[i] + er_extero[i] + meta_prior * (data["top_kl"][k][1].detach().numpy() + data["prop_kl"][k][0].detach().numpy() + data["vision_kl"][k][0].detach().numpy())
        

        for i in range(fe.shape[0]):
            if i == 0:
                fe_moving_average[i] = np.mean(fe[i])
            elif i < 19:
                fe_moving_average[i] = np.mean(fe[:i])
            else:
                fe_moving_average[i] = np.mean(fe[i-19:i])
	...
	#free energy
        axes[6].plot(fe_moving_average, linestyle="solid", linewidth="1", color="k")
        self.add_info(axes[6], None, None, (0.0, 0.1), None, "Free energy")
        
        #Real sensations
        axes[7].plot(target_state[:, 2:], linestyle="solid", linewidth="1", color="r") #extero
        axes[7].plot(target_state[:, :2], linestyle="solid", linewidth="1", color="b") #prop
            ...
            for k, foldername_ite in enumerate(foldername_list_ite):
                ...
                for i, foldername_window in enumerate(foldername_list_window):
                    ...
                    for j, foldername_lr in enumerate(foldername_list_lr):
                        ...
                        plot.state(SLICE, T_MIN, T_MAX, 8, 2)
